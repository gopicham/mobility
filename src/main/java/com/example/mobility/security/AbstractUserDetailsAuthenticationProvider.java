 ...
 * objects. The class is designed to respond to
 * {@link UsernamePasswordAuthenticationToken} authentication requests.
 *
 * <p>
 * Upon successful validation, a
 * <code>UsernamePasswordAuthenticationToken</code> will be created and returned
 * to the caller. The token will include as its principal either a
 * <code>String</code> representation of the username, or the
 * {@link UserDetails} that was returned from the authentication repository.
 * Using <code>String</code> is appropriate if a container adapter is being
 * used, as it expects <code>String</code> representations of the username.
 * Using <code>UserDetails</code> is appropriate if you require access to
 ...
 * <code>true</code>.
 * <p>
 * Caching is handled by storing the <code>UserDetails</code> object being
 * placed in the {@link UserCache}. This ensures that subsequent requests with
 * the same username can be validated without needing to query the
 * {@link UserDetailsService}. It should be noted that if a user appears to
 * present an incorrect password, the {@link UserDetailsService} will be queried
 * to confirm the most up-to-date password was used for comparison. Caching is
 * only likely to be required for stateless applications. In a normal web
 * application, for example, the <tt>SecurityContext</tt> is stored in the
 * user's session and the user isn't reauthenticated on each request. The
...
public abstract class AbstractUserDetailsAuthenticationProvider
	 ...
	 * <code>UsernamePasswordAuthenticationToken</code>, these should also appear in
	 * this method.
	 * 
	 ...
	 *                       (defined by the {@link #isForcePrincipalAsString()}
	 *                       method)
	 ...
	 *                       <em>may</em> need to perform a binding-based retrieval
	 *                       of the <code>UserDetails</code>
	 ...
	 *                                   <code>UsernameNotFoundException</code>s to
	 *                                   be thrown instead of the non-specific
